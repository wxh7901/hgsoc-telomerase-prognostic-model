          ...
          ES_geom = "line") 
{
  ...
  #添加NES,P.adj,FDR
  if(NES_table==TRUE){
      p.res <- p.res + annotate("text", x = max(p.res$data$x)*0.75+min(p.res$data$x)*0.25, y = max(p.res$data$runningScore)*0.75+min(p.res$data$runningScore)*0.25, 
                                label = paste0('NES = ',round(x[geneSetID,"NES"],4),'\n',
                                               "P.adjust = ",round(x[geneSetID,"p.adjust"],4),'\n',
                                               'FDR = ',round(x[geneSetID,"qvalues"],4)),
                                size = 2)
  }
